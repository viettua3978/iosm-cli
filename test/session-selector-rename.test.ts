...
describe("session selector rename", () => {
	...
	it("enters rename mode on Ctrl+R and submits with Enter", async () => {
		...
		expect(output).toContain("Current name: Old");
	...
	it("shows search label and session summary in list mode", async () => {
		const sessions = [makeSession({ id: "a", name: "Old" })];
		const selector = new SessionSelectorComponent(
			async () => sessions,
			async () => [],
		...
		const output = selector.render(120).join("\n");
		expect(output).toContain("Search sessions");
		expect(output).toContain("Showing 1/1");
