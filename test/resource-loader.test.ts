...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".iosm", "prompts");
			...
			const projectSkillDir = join(cwd, ".iosm", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".iosm", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".iosm", "themes"), { recursive: true });
		...
		it("should keep both extensions loaded when command names collide", async () => {
			...
			const projectExtDir = join(cwd, ".iosm", "extensions");
...
Content`,
		...
		it("should load both AGENTS.md and IOSM.md context from the same directory", async () => {
			writeFileSync(join(cwd, "AGENTS.md"), "# Project Guidelines\n\nBe helpful.");
			writeFileSync(join(cwd, "IOSM.md"), "# IOSM.md\n\nFocus on logic first.");

			const loader = new DefaultResourceLoader({ cwd, agentDir, contextProfile: "iosm" });
			await loader.reload();

			const { agentsFiles } = loader.getAgentsFiles();
			const agentsFile = agentsFiles.find((f) => f.path.endsWith("AGENTS.md"));
			const iosmFile = agentsFiles.find((f) => f.path.endsWith("IOSM.md"));

			expect(agentsFile?.content).toContain("# Project Guidelines");
			expect(iosmFile?.content).toContain("<iosm-context>");
		...
		it("should skip IOSM.md context outside iosm profile", async () => {
			...
			const loader = new DefaultResourceLoader({ cwd, agentDir, contextProfile: "standard" });
			await loader.reload();
		...
		it("should load hierarchical .iosm memory from organization, home, and project scopes", async () => {
			...
			cwd = join(workspaceRoot, "apps", "demo");
			mkdirSync(cwd, { recursive: true });

			const orgConfigDir = join(orgDir, ".iosm");
			mkdirSync(join(orgConfigDir, "memory"), { recursive: true });
			writeFileSync(join(orgConfigDir, "memory.md"), "# Org memory");
			writeFileSync(join(orgConfigDir, "memory", "rules.md"), "Org rule");

			const homeConfigDir = join(homeDir, ".iosm");
			mkdirSync(join(homeConfigDir, "memory", "shared"), { recursive: true });
			writeFileSync(join(homeConfigDir, "memory.md"), "# Home memory\n\n@import ./memory/shared/home.md");
			writeFileSync(join(homeConfigDir, "memory", "shared", "home.md"), "Home imported rule");

			const projectConfigDir = join(workspaceRoot, ".iosm");
			mkdirSync(join(projectConfigDir, "memory"), { recursive: true });
			writeFileSync(join(projectConfigDir, "memory.md"), "# Project memory");
			writeFileSync(join(projectConfigDir, "memory", "project.md"), "Project rule");

			const localConfigDir = join(cwd, ".iosm");
			mkdirSync(localConfigDir, { recursive: true });
			writeFileSync(join(localConfigDir, "memory.md"), "# Local memory");
			...
			expect(paths).toContain(join(orgConfigDir, "memory.md"));
			expect(paths).toContain(join(homeConfigDir, "memory.md"));
			expect(paths).toContain(join(projectConfigDir, "memory.md"));
			expect(paths).toContain(join(localConfigDir, "memory.md"));

			const homeMemory = loader.getAgentsFiles().agentsFiles.find((f) => f.path === join(homeConfigDir, "memory.md"));
			expect(homeMemory?.content).toContain("Home imported rule");
		});

		it("should load agent-scoped memory files from agentDir root", async () => {
			mkdirSync(join(agentDir, "memory", "nested"), { recursive: true });
			writeFileSync(join(agentDir, "memory.md"), "# Agent memory");
			writeFileSync(join(agentDir, "memory", "nested", "rule.md"), "Agent nested rule");

			const loader = new DefaultResourceLoader({ cwd, agentDir });
			await loader.reload();

			const paths = loader.getAgentsFiles().agentsFiles.map((f) => f.path);
			expect(paths).toContain(join(agentDir, "memory.md"));
			expect(paths).toContain(join(agentDir, "memory", "nested", "rule.md"));
		});

		it("should discover SYSTEM.md from cwd/.iosm", async () => {
			const piDir = join(cwd, ".iosm");
		...
		it("should load hooks from .iosm/hooks.json", async () => {
			const configDir = join(cwd, ".iosm");
			...
			expect(hooks.sources).toContain(join(configDir, "hooks.json"));
			expect(hooks.preToolUse.some((rule) => rule.id === "deny-rm")).toBe(true);
		});
	});
