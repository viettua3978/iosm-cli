...
function parseOrchestrateBlock(text: string):
	...
	| undefined {
	const match = text.match(/<orchestrate\b([^>]*)>([\s\S]*?)<\/orchestrate>/i);
	...
	const maxParallelRaw = readAttr("max_parallel");
	const agentsParsed = agentsRaw ? Number.parseInt(agentsRaw, 10) : undefined;
	const maxParallelParsed = maxParallelRaw ? Number.parseInt(maxParallelRaw, 10) : undefined;

	return {
		mode,
		agents: Number.isInteger(agentsParsed) ? agentsParsed : undefined,
		maxParallel: Number.isInteger(maxParallelParsed) ? maxParallelParsed : undefined,
		body,
	};
}

function deriveOrchestrationDisplayText(promptTextWithoutDirective: string): string | undefined {
	const block = parseOrchestrateBlock(promptTextWithoutDirective);
	if (!block) return undefined;
	const taskMatch = block.body.match(/(?:^|\n)\s*task:\s*(.+)$/im);
	const task = taskMatch?.[1]?.trim();
	if (!task) return undefined;
	const agentMatch = block.body.match(/(?:^|\n)\s*-\s*agent\s+\d+:[^\n]*\bagent=([A-Za-z0-9._-]+)/im);
...
function buildSubagentOrchestrationDirective(text: string): string | undefined {
	...
	if (block) {
		return [
			...
			"The user provided an explicit orchestration contract via <orchestrate>.",
			"You MUST execute this contract using the `task` tool.",
			"Before final prose, perform the required task tool calls from this contract.",
			`Execution mode: ${block.mode ?? "unspecified"}.`,
			`Requested subagent count: ${block.agents ?? "unspecified"}.`,
			`Max parallel workers: ${block.maxParallel ?? "unspecified"}.`,
			"Use one task call per agent assignment in the contract body.",
			"Each task call must include run_id and task_id when they are present in the assignment line.",
			"When parallel mode is requested, launch independent task calls up to max_parallel if provided.",
			"For parallel mode, emit independent task calls in one assistant response when possible.",
			"In parallel mode, follow parallel tool-call style and batch independent task calls together in the same turn.",
			"For write-capable parallel agents, include lock_key only when serialization is required; omit lock_key for independent read-only analyses.",
			"For orchestration contracts, keep required task calls foreground and avoid background=true unless the user explicitly asked for detached async execution.",
			"During orchestration execution, emit task calls and final synthesis only; avoid interim status prose and avoid manual polling via bash/read.",
...
export type AgentSessionEvent =
	...
	| { type: "meta_applied"; step: number; text: string };
...
export interface AgentSessionConfig {
	...
	/**
	 * Extra text appended to the system prompt (after extensions).
	 * Used to inject profile-specific instructions for the main session.
	 */
...
export interface PromptOptions {
	...
	/** Image attachments */
	images?: ImageContent[];
	/** Skip automatic IOSM runtime context injection for internal/system prompts. */
	skipIosmAutopilot?: boolean;
	/** When streaming, how to queue the message: "steer" (interrupt), "followUp" (wait), or "meta" (adaptive soft/pivot update). Required if streaming. */
	streamingBehavior?: "steer" | "followUp" | "meta";
...
export class AgentSession {
	...
	/** Tracks pending meta messages for UI display and task-level checkpoint polling. */
	private _metaMessages: string[] = [];
	/** Monotonic counter for applied meta updates (displayed as "applied at step X"). */
	private _metaAppliedStep = 0;
	...
	private _emitConfigChange(event: Omit<ConfigChangeEvent, "type">): void {
	...
	private _refreshSessionTracePath(reason: string): void {
		if (!this._sessionTraceEnabled) {
			this._sessionTracePath = undefined;
			return;
		}

		const sessionId = this.sessionManager.getSessionId();
		const nextPath = getSessionTracePath(sessionId);
		this._sessionTracePath = nextPath;
	...
	private _appendSessionTrace(payload: Record<string, unknown>): void {
		if (!this._sessionTraceEnabled || !this._sessionTracePath) {
			return;
		}
		...
		try {
			appendFileSync(
				this._sessionTracePath,
				`${JSON.stringify(entry, (_key, value) => (typeof value === "bigint" ? value.toString() : value))}\n`,
				"utf8",
			);
		} catch {
			// Tracing is best-effort and must not break the session runtime.
		}
	}

	private _queueHookNotices(event: HookEventName, notices: string[]): void {
		for (const notice of notices) {
			const trimmed = notice.trim();
			if (trimmed.length === 0) continue;
			this._pendingHookNotices.push({ event, message: trimmed });
		}
	}

	private _flushHookNotices(): void {
		if (this._pendingHookNotices.length === 0) return;
		const notices = [...this._pendingHookNotices];
		this._pendingHookNotices = [];
		for (const notice of notices) {
			this._appendCustomMessageLocally({
				customType: "hooks",
				content: `[${notice.event}] ${notice.message}`,
				display: true,
			});
		}
	}

	private _extractToolResultText(result: unknown): string {
		if (!isRecord(result)) {
			return "";
		}
	...
	private async _processAgentEvent(event: AgentEvent): Promise<void> {
		...
		if (event.type === "message_end" && event.message.role === "assistant") {
			...
			if (extracted.changed) {
				(event.message as AssistantMessage).content = extracted.sanitizedMessage.content;
			}
		}

		// When a user message starts, check if it's from either queue and remove it BEFORE emitting
		// This ensures the UI sees the updated queue state
		if (processedEvent.type === "message_start" && processedEvent.message.role === "user") {
			this._overflowRecoveryAttempted = false;
			const messageText = this._getUserMessageText(processedEvent.message);
			if (messageText) {
				const metaIndex = this._metaMessages.indexOf(messageText);
				if (metaIndex !== -1) {
					this._metaMessages.splice(metaIndex, 1);
					this._metaAppliedStep += 1;
					this._emit({ type: "meta_applied", step: this._metaAppliedStep, text: messageText });
				} else {
		...
		await this._emitExtensionEvent(processedEvent);

		// Notify all listeners
		this._emit(processedEvent);

		// Handle session persistence
		if (processedEvent.type === "message_end") {
			// Check if this is a custom message from extensions
			if (processedEvent.message.role === "custom") {
				// Persist as CustomMessageEntry
				this.sessionManager.appendCustomMessageEntry(
					processedEvent.message.customType,
					processedEvent.message.content,
					processedEvent.message.display,
					processedEvent.message.details,
				);
			} else if (
				processedEvent.message.role === "user" ||
				processedEvent.message.role === "assistant" ||
				processedEvent.message.role === "toolResult"
			) {
				// Regular LLM message - persist as SessionMessageEntry
				this.sessionManager.appendMessage(processedEvent.message);
			}
			// Other message types (bashExecution, compactionSummary, branchSummary) are persisted elsewhere

			// Track assistant message for auto-compaction (checked on agent_end)
			if (processedEvent.message.role === "assistant") {
				this._lastAssistantMessage = processedEvent.message;

				const assistantMsg = processedEvent.message as AssistantMessage;
				...
				if (taskPlanSnapshot) {
					const signature = taskPlanSignature(taskPlanSnapshot);
					if (signature !== this._lastTaskPlanSignature) {
						this._lastTaskPlanSignature = signature;
						this._appendCustomMessageLocally({
							customType: TASK_PLAN_CUSTOM_TYPE,
							content: formatTaskPlanMessageContent(taskPlanSnapshot),
							display: true,
							details: taskPlanSnapshot,
						});
					}
				}
			}
		...
		if (processedEvent.type === "agent_end" && this._lastAssistantMessage) {
	...
	private async _emitExtensionEvent(event: AgentEvent): Promise<void> {
		...
		if (event.type === "agent_start") {
			...
			if (event.toolName === "task") {
				...
				this._subagentStartMeta.set(event.toolCallId, {
					startedAt: Date.now(),
					profile,
					description,
					cwd: args.cwd,
				...
				const subagentStart: SubagentStartEvent = {
					type: "subagent_start",
					toolCallId: event.toolCallId,
					description,
					profile,
				...
				await this._extensionRunner.emit(subagentStart);
			}
			...
			await this._extensionRunner.emit(extensionEvent);
			if (event.toolName === "task") {
				const startMeta = this._subagentStartMeta.get(event.toolCallId);
				this._subagentStartMeta.delete(event.toolCallId);
				...
				const status: "done" | "error" | "cancelled" = event.isError ? "error" : "done";
				const errorMessage = event.isError
					? (event.result?.content?.[0] as { text?: string } | undefined)?.text
					: undefined;
				const durationMs = Math.max(0, Date.now() - (startMeta?.startedAt ?? Date.now()));
				const profile = startMeta?.profile ?? "full";
				const description = startMeta?.description ?? "Running subtask";
				const subagentStop: SubagentStopEvent = {
					type: "subagent_stop",
					toolCallId: event.toolCallId,
					description,
					profile,
					...
					errorMessage,
					cwd: startMeta?.cwd,
					agent: startMeta?.agent,
					lockKey: startMeta?.lockKey,
					isolation: startMeta?.isolation,
				};
				await this._extensionRunner.emit(subagentStop);
				...
				await this._extensionRunner.emit(taskCompleted);
				if (this._subagentStartMeta.size === 0) {
					const teammateIdle: TeammateIdleEvent = {
						type: "teammate_idle",
						activeSubagents: 0,
						lastCompletedToolCallId: event.toolCallId,
					};
					await this._extensionRunner.emit(teammateIdle);
				}
			}
		}
	}
	...
	setActiveToolsByName(toolNames: string[]): void {
		...
		this._emitConfigChange({
			...
			current: validToolNames,
		});
	}

	/**
	 * Set or clear the profile-specific system prompt suffix.
	 * Rebuilds and reapplies the base system prompt immediately.
	 */
	setSystemPromptSuffix(suffix?: string): void {
		const previousSuffix = this._systemPromptSuffix;
		const nextSuffix = suffix?.trim();
		this._systemPromptSuffix = nextSuffix && nextSuffix.length > 0 ? nextSuffix : undefined;
		this._baseSystemPrompt = this._rebuildSystemPrompt(this.getActiveToolNames());
		this.agent.setSystemPrompt(this._baseSystemPrompt);
		this._emitConfigChange({
			key: "system_prompt_suffix",
			previous: previousSuffix,
			current: this._systemPromptSuffix,
		});
	}

	setIosmAutopilotEnabled(enabled: boolean): void {
		const previous = this._iosmAutopilotEnabled;
		this._iosmAutopilotEnabled = enabled;
		this._baseSystemPrompt = this._rebuildSystemPrompt(this.getActiveToolNames());
		this.agent.setSystemPrompt(this._baseSystemPrompt);
		this._emitConfigChange({
			key: "iosm_autopilot_enabled",
			previous,
			current: enabled,
		});
	}
	...
	/** Whether full session trace logging is currently enabled for this session. */
	get isSessionTraceOn(): boolean {
		return this._sessionTraceEnabled;
	}

	/** Absolute path to the current session trace JSONL file, when tracing is enabled. */
	get sessionTracePath(): string | undefined {
		return this._sessionTracePath;
	}

	/** Enable full session trace logging for the current runtime session. */
	enableSessionTrace(): void {
		if (this._sessionTraceEnabled) {
			return;
		}
	...
	private _rebuildSystemPrompt(toolNames: string[]): string {
		...
		if (this._systemPromptSuffix) allAppends.push(this._systemPromptSuffix);
		const appendSystemPrompt = allAppends.length > 0 ? allAppends.join("\n\n") : undefined;
		const loadedSkills = this._resourceLoader.getSkills().skills;
		const loadedContextFiles = this._resourceLoader
			.getAgentsFiles()
	 ...
	 * - Handles extension commands (registered via iosm.registerCommand) immediately, even during streaming
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		const userPromptHooks = applyUserPromptSubmitHooks(this._hooksConfig, currentText);
		this._queueHookNotices("UserPromptSubmit", userPromptHooks.notices);
		if (userPromptHooks.notices.length > 0) {
			this._flushHookNotices();
		}
		if (userPromptHooks.blocked) {
			this._flushHookNotices();
			throw new Error(userPromptHooks.message ?? "Prompt blocked by UserPromptSubmit hook.");
		}
		currentText = userPromptHooks.text;
		...
		await this._injectIosmRuntimeContext(currentText, "nextTurn", {
			source: inputSource,
			skip: options?.skipIosmAutopilot,
		});

		// Build messages array (custom context first, then user message)
	...
	async steer(text: string, images?: ImageContent[]): Promise<void> {
		...
		await this._injectIosmRuntimeContext(text, "steer", {
			source: "interactive",
		});
	...
	async followUp(text: string, images?: ImageContent[]): Promise<void> {
		...
		await this._injectIosmRuntimeContext(text, "followUp", {
			source: "interactive",
		});
	...
	/**
	 * Queue a meta message as a soft update.
	 * It does not interrupt current tool execution and is delivered like follow-up,
	 * while being tracked separately for UI and task-level checkpoint polling.
	 */
	async meta(text: string, images?: ImageContent[]): Promise<void> {
		// Check for extension commands (cannot be queued)
		if (text.startsWith("/")) {
			this._throwIfExtensionCommand(text);
		}

		await this._injectIosmRuntimeContext(text, "meta", {
			source: "interactive",
		});
		...
		await this._queueMeta(expandedText, images);
	}
	...
	/**
	 * Internal: Queue a meta message.
	 * Uses steering delivery while streaming so updates are applied at the next tool boundary
	 * without language-specific trigger parsing. We still keep a separate meta queue for UI/polling.
	 */
	private async _queueMeta(text: string, images?: ImageContent[]): Promise<void> {
		this._metaMessages.push(text);
		this._appendSessionTrace({
			type: "queue_meta",
			text,
			imageCount: images?.length ?? 0,
		...
		// Language-agnostic behavior: while streaming, meta always pivots via steer.
		// This avoids brittle keyword trigger lists across locales.
		const deliverAsSteer = this.isStreaming;
		const queueMethod = deliverAsSteer ? this.agent.steer.bind(this.agent) : this.agent.followUp.bind(this.agent);
		queueMethod({
			role: "user",
			content,
			timestamp: Date.now(),
	...
	private _shouldInjectIosmRuntimeContext(options: { source?: InputSource; skip?: boolean }): boolean {
		if (options.skip) {
			return false;
		}
		if (!this._iosmAutopilotEnabled) {
			return false;
		}
		return options.source !== "extension";
	}

	private async _injectIosmRuntimeContext(
		userInput: string,
		deliverAs: "steer" | "followUp" | "meta" | "nextTurn",
		options: { source?: InputSource; skip?: boolean },
	): Promise<void> {
		if (!this._shouldInjectIosmRuntimeContext(options)) {
			return;
		}

		const runtimeContext = await prepareIosmRuntimeContext(this._cwd, userInput);
		await this.sendCustomMessage(
			{
				customType: "iosm-runtime",
				content: buildIosmRuntimeDirective(runtimeContext),
				display: false,
			},
			{ deliverAs },
		);
	 ...
	 * @param options.deliverAs Delivery mode: "steer", "followUp", "meta", or "nextTurn"
	 */
	private _appendCustomMessageLocally<T = unknown>(
	...
	async sendCustomMessage<T = unknown>(
		message: Pick<CustomMessage<T>, "customType" | "content" | "display" | "details">,
		options?: { triggerTurn?: boolean; deliverAs?: "steer" | "followUp" | "meta" | "nextTurn" },
	): Promise<void> {
		const appMessage = {
			role: "custom" as const,
			customType: message.customType,
		...
		if (options?.deliverAs === "nextTurn") {
			...
			if (options?.deliverAs === "followUp" || options?.deliverAs === "meta") {
			...
			this._appendCustomMessageLocally(message);
		}
	}
	 ...
	 * @param options.deliverAs Delivery mode when streaming: "steer", "followUp", or "meta"
	 */
	async sendUserMessage(
		content: string | (TextContent | ImageContent)[],
		options?: { deliverAs?: "steer" | "followUp" | "meta" },
	): Promise<void> {
	 ...
	 * @returns Object with steering, followUp, and meta arrays
	 */
	clearQueue(): { steering: string[]; followUp: string[]; meta: string[] } {
		const steering = [...this._steeringMessages];
		const followUp = [...this._followUpMessages];
		const meta = [...this._metaMessages];
		this._steeringMessages = [];
		this._followUpMessages = [];
		this._metaMessages = [];
		this.agent.clearAllQueues();
		return { steering, followUp, meta };
	}

	/** Number of pending messages (includes steering, follow-up, and meta) */
	get pendingMessageCount(): number {
		return this._steeringMessages.length + this._followUpMessages.length + this._metaMessages.length;
	}
	...
	/** Get pending meta messages (read-only). */
	getMetaMessages(): readonly string[] {
		return this._metaMessages;
	}

	/** Number of meta updates already applied into the run. */
	get metaAppliedStep(): number {
		return this._metaAppliedStep;
	}
	...
	async notifyStopHooks(reason: string): Promise<void> {
		const notices = applyStopHooks(this._hooksConfig, reason);
		if (notices.length === 0) return;
		this._queueHookNotices("Stop", notices);
		if (!this.isStreaming) {
			this._flushHookNotices();
		}
	}
	...
	async abort(): Promise<void> {
		await this.notifyStopHooks("abort");
		this.abortRetry();
		this.agent.abort();
	...
	async newSession(options?: {
		...
		this.agent.sessionId = this.sessionManager.getSessionId();
			this._refreshSessionTracePath("new_session");
			this._steeringMessages = [];
			this._followUpMessages = [];
			this._metaMessages = [];
			this._metaAppliedStep = 0;
			this._pendingNextTurnMessages = [];
	...
	setSteeringMode(mode: "all" | "one-at-a-time"): void {
		const previous = this.agent.getSteeringMode();
	...
	setFollowUpMode(mode: "all" | "one-at-a-time"): void {
		const previous = this.agent.getFollowUpMode();
	...
	/** Default behavior for Enter while streaming in interactive mode. */
	get streamInputMode(): "steer" | "followUp" | "meta" {
		return this.settingsManager.getStreamInputMode();
	}

	setStreamInputMode(mode: "steer" | "followUp" | "meta"): void {
		const previous = this.settingsManager.getStreamInputMode();
		this.settingsManager.setStreamInputMode(mode);
		if (previous !== mode) {
			this._emitConfigChange({
				key: "stream_input_mode",
				previous,
				current: mode,
			});
		}
	...
	setAutoCompactionEnabled(enabled: boolean): void {
		const previous = this.settingsManager.getCompactionEnabled();
		this.settingsManager.setCompactionEnabled(enabled);
		if (previous !== enabled) {
			this._emitConfigChange({
				key: "auto_compaction_enabled",
				previous,
				current: enabled,
			});
		}
	...
	setAutoRetryEnabled(enabled: boolean): void {
		const previous = this.settingsManager.getRetryEnabled();
	...
	async switchSession(sessionPath: string): Promise<boolean> {
		...
		this.agent.sessionId = this.sessionManager.getSessionId();
		this._refreshSessionTracePath("switch_session");
	...
	async fork(entryId: string): Promise<{ selectedText: string; cancelled: boolean }> {
		...
		this.agent.sessionId = this.sessionManager.getSessionId();
		this._refreshSessionTracePath("fork_session");
