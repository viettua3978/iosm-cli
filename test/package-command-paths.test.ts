...
describe("package commands", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `iosm-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("iosm install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "iosm --help" or "iosm install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: iosm install <source> [-l]");
