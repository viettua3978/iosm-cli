...
function collectAncestorResourceDirs(startDir: string, topLevelDirName: string, resourceDirName: string): string[] {
	...
	while (true) {
		resourceDirs.push(join(dir, topLevelDirName, resourceDirName));
...
function readPiManifestFile(packageJsonPath: string): PiManifest | null {
	try {
		...
		const pkg = JSON.parse(content) as { iosm?: PiManifest; pi?: PiManifest };
		return pkg.iosm ?? null;
	} catch {
		return null;
	}
}
...
export class DefaultPackageManager implements PackageManager {
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "iosm-extensions", private: true };
	...
	private getTemporaryDir(prefix: string, suffix?: string): string {
		const hash = createHash("sha256")
			...
			.digest("hex")
			.slice(0, 8);
		return join(tmpdir(), "iosm-extensions", prefix, hash, suffix ?? "");
	}
	...
	private addAutoDiscoveredResources(
		...
		const userOpenCodeSkillsDir = join(homedir(), ".config", "opencode", "skills");
		const userClaudeCommandsDir = join(homedir(), ".claude", "commands");
		const userOpenCodeCommandsDir = join(homedir(), ".config", "opencode", "commands");
		const projectAgentsSkillDirs = collectAncestorResourceDirs(this.cwd, ".agents", "skills");
		const projectClaudeSkillDirs = collectAncestorResourceDirs(this.cwd, ".claude", "skills");
		const projectOpenCodeSkillDirs = collectAncestorResourceDirs(this.cwd, ".opencode", "skills");
		const projectClaudeCommandDirs = collectAncestorResourceDirs(this.cwd, ".claude", "commands");
		const projectOpenCodeCommandDirs = collectAncestorResourceDirs(this.cwd, ".opencode", "commands");
		...
		addResources(
			...
			[
				...
				...projectOpenCodeSkillDirs.flatMap((dir) => collectAutoSkillEntries(dir)),
			],
			projectMetadata,
		...
		addResources(
			...
			[
				...
				...projectClaudeCommandDirs.flatMap((dir) => collectAutoPromptEntries(dir)),
				...projectOpenCodeCommandDirs.flatMap((dir) => collectAutoPromptEntries(dir)),
			],
			projectMetadata,
		...
		addResources(
			...
			[
				...
				...collectAutoSkillEntries(userClaudeSkillsDir),
				...collectAutoSkillEntries(userOpenCodeSkillsDir),
			],
			userMetadata,
		...
		addResources(
			...
			[
				...
				...collectAutoPromptEntries(userClaudeCommandsDir),
				...collectAutoPromptEntries(userOpenCodeCommandsDir),
			],
			userMetadata,
