...
describe("parseArgs", () => {
	...
	describe("flags with values", () => {
		...
		test("parses --session-trace-dir", () => {
			const result = parseArgs(["--session-trace-dir", "/tmp/iosm-traces"]);
			expect(result.sessionTraceDir).toBe("/tmp/iosm-traces");
		});
	});
	...
	describe("--session-trace flag", () => {
		test("parses --session-trace", () => {
			const result = parseArgs(["--session-trace"]);
			expect(result.sessionTrace).toBe(true);
		});
	});
