...
function bashBadge(label: string, color: "bashMode" | "success" | "warning" | "error" | "muted"): string {
...
export class BashExecutionComponent extends Container {
	...
	constructor(command: string, ui: TUI, excludeFromContext = false) {
		...
		this.loader = new Loader(
			...
			`bash running (${editorKey("selectCancel")} to cancel)`, // Plain text for loader
		);
	...
	private updateDisplay(): void {
		...
		// Command header
		const scopeBadge = this.excludeFromContext ? bashBadge("no-context", "muted") : bashBadge("bash", "bashMode");
		let headerMeta = "";
		if (this.status === "running") {
			headerMeta = theme.fg("muted", "(running)");
		} else if (this.status === "complete") {
			headerMeta = theme.fg("success", "(done)");
		} else if (this.status === "cancelled") {
			headerMeta = theme.fg("warning", "(cancelled)");
		} else if (this.status === "error") {
			headerMeta = theme.fg("error", `(exit ${this.exitCode ?? "?"})`);
		}
		const header = new Text(
			`${scopeBadge} ${theme.fg(colorKey, theme.bold(`$ ${this.command}`))}${headerMeta ? ` ${headerMeta}` : ""}`,
			1,
			0,
		);
		this.contentContainer.addChild(header);
		...
		if (this.status === "running") {
			...
			if (hiddenLineCount > 0) {
				if (this.expanded) {
					statusParts.push(theme.fg("muted", `(${keyHint("expandTools", "to collapse")})`));
				} else {
					statusParts.push(
