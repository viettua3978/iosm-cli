...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const cachedDir = join(tmpdir(), "iosm-extensions", `git-${gitHost}`, hash, gitPath);
			const extensionFile = join(cachedDir, "iosm-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "iosm-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ iosm: { extensions: ["./iosm-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "iosm-extensions/session-breakdown.ts")).toBe("// fresh");
		});
		...
		it("should not refresh pinned temporary git sources", async () => {
			...
			expect(getFileContent(cachedDir, "iosm-extensions/session-breakdown.ts")).toBe("// pinned");
		});
	});
	...
	describe("scope-aware update", () => {
		it("should not install locally when source is only registered globally", async () => {
			...
			const projectGitDir = join(tempDir, ".iosm", "git", "github.com", "test", "extension");
