...
describe("DefaultPackageManager", () => {
	...
	describe("resolve", () => {
		it("should return no package-sourced paths when no sources configured", async () => {
			...
			expect(result.prompts.every((r) => r.metadata.source === "auto" && r.metadata.origin === "top-level")).toBe(
				true,
			);
...
Content`,
		...
		it("should resolve project paths relative to .iosm", async () => {
			const extDir = join(tempDir, ".iosm", "extensions");
			mkdirSync(extDir, { recursive: true });
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".iosm", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should auto-discover .claude/commands and .opencode/commands from cwd up to git repo root", async () => {
			const repoRoot = join(tempDir, "repo");
			const nestedCwd = join(repoRoot, "apps", "web");
			mkdirSync(nestedCwd, { recursive: true });
			mkdirSync(join(repoRoot, ".git"), { recursive: true });

			const aboveRepoClaudeCommand = join(tempDir, ".claude", "commands", "above-repo.md");
			mkdirSync(join(tempDir, ".claude", "commands"), { recursive: true });
			writeFileSync(aboveRepoClaudeCommand, "Above repo command");

			const repoClaudeCommand = join(repoRoot, ".claude", "commands", "repo-claude.md");
			mkdirSync(join(repoRoot, ".claude", "commands"), { recursive: true });
			writeFileSync(repoClaudeCommand, "Repo claude command");

			const nestedClaudeCommand = join(repoRoot, "apps", ".claude", "commands", "nested-claude.md");
			mkdirSync(join(repoRoot, "apps", ".claude", "commands"), { recursive: true });
			writeFileSync(nestedClaudeCommand, "Nested claude command");

			const repoOpenCodeCommand = join(repoRoot, ".opencode", "commands", "repo-opencode.md");
			mkdirSync(join(repoRoot, ".opencode", "commands"), { recursive: true });
			writeFileSync(repoOpenCodeCommand, "Repo opencode command");

			const nestedOpenCodeCommand = join(repoRoot, "apps", ".opencode", "commands", "nested-opencode.md");
			mkdirSync(join(repoRoot, "apps", ".opencode", "commands"), { recursive: true });
			writeFileSync(nestedOpenCodeCommand, "Nested opencode command");
			...
			expect(result.prompts.some((r) => r.path === repoClaudeCommand && r.enabled)).toBe(true);
			expect(result.prompts.some((r) => r.path === nestedClaudeCommand && r.enabled)).toBe(true);
			expect(result.prompts.some((r) => r.path === repoOpenCodeCommand && r.enabled)).toBe(true);
			expect(result.prompts.some((r) => r.path === nestedOpenCodeCommand && r.enabled)).toBe(true);
			expect(result.prompts.some((r) => r.path === aboveRepoClaudeCommand)).toBe(false);
		});

		it("should resolve directory with package.json iosm.extensions in extensions setting", async () => {
			// Create a package with iosm.extensions in package.json
			...
			// Should find the extensions declared in package.json iosm.extensions
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should scan .claude/skills and .opencode/skills from cwd up to git repo root", async () => {
			const repoRoot = join(tempDir, "repo");
			const nestedCwd = join(repoRoot, "apps", "web");
			mkdirSync(nestedCwd, { recursive: true });
			mkdirSync(join(repoRoot, ".git"), { recursive: true });

			const aboveRepoClaudeSkill = join(tempDir, ".claude", "skills", "above-repo", "SKILL.md");
			mkdirSync(join(tempDir, ".claude", "skills", "above-repo"), { recursive: true });
			writeFileSync(aboveRepoClaudeSkill, "---\nname: above-repo\ndescription: above repo\n---\n");

			const repoClaudeSkill = join(repoRoot, ".claude", "skills", "repo-claude", "SKILL.md");
			mkdirSync(join(repoRoot, ".claude", "skills", "repo-claude"), { recursive: true });
			writeFileSync(repoClaudeSkill, "---\nname: repo-claude\ndescription: repo claude\n---\n");

			const nestedClaudeSkill = join(repoRoot, "apps", ".claude", "skills", "nested-claude", "SKILL.md");
			mkdirSync(join(repoRoot, "apps", ".claude", "skills", "nested-claude"), { recursive: true });
			writeFileSync(nestedClaudeSkill, "---\nname: nested-claude\ndescription: nested claude\n---\n");

			const repoOpenCodeSkill = join(repoRoot, ".opencode", "skills", "repo-opencode", "SKILL.md");
			mkdirSync(join(repoRoot, ".opencode", "skills", "repo-opencode"), { recursive: true });
			writeFileSync(repoOpenCodeSkill, "---\nname: repo-opencode\ndescription: repo opencode\n---\n");

			const nestedOpenCodeSkill = join(repoRoot, "apps", ".opencode", "skills", "nested-opencode", "SKILL.md");
			mkdirSync(join(repoRoot, "apps", ".opencode", "skills", "nested-opencode"), { recursive: true });
			writeFileSync(nestedOpenCodeSkill, "---\nname: nested-opencode\ndescription: nested opencode\n---\n");
			...
			const result = await pm.resolve();
			expect(result.skills.some((r) => r.path === repoClaudeSkill && r.enabled)).toBe(true);
			expect(result.skills.some((r) => r.path === nestedClaudeSkill && r.enabled)).toBe(true);
			expect(result.skills.some((r) => r.path === repoOpenCodeSkill && r.enabled)).toBe(true);
			expect(result.skills.some((r) => r.path === nestedOpenCodeSkill && r.enabled)).toBe(true);
			expect(result.skills.some((r) => r.path === aboveRepoClaudeSkill)).toBe(false);
		});
	});
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .iosm auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".iosm\n");

			const skillDir = join(tempDir, ".iosm", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with iosm manifest", async () => {
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .iosm settings base", () => {
			...
			const rel = relative(join(tempDir, ".iosm"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in iosm manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json iosm.extensions manifest in subdirectories", async () => {
