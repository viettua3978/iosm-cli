 ...
 * - ~/.iosm/agent/presets.json (global)
 * - <cwd>/.iosm/presets.json (project-local)
 *
...
import type { ExtensionAPI, ExtensionContext } from "iosm-cli";
import { DynamicBorder } from "iosm-cli";
...
function loadPresets(cwd: string): PresetsConfig {
	const globalPath = join(homedir(), ".iosm", "agent", "presets.json");
	const projectPath = join(cwd, ".iosm", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.iosm/agent/presets.json or .iosm/presets.json", "warning");
			return;
		}
