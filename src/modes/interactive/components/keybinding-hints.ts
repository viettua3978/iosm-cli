...
// Compact hint bracket format: [key desc]
function bracketHint(key: string, desc: string): string {
	return theme.fg("dim", "[") + theme.fg("dim", key) + " " + theme.fg("muted", desc) + theme.fg("dim", "]");
}
...
/**
 * Render a compact single-line keybinding hints bar in Claude Code style.
 * Shows only the most important actions in bracket notation:
 *   [↵ send]  [⌃c stop]  [↑↓ history]  [⌃k clear]
 *
 * @param keybindings - KeybindingsManager for app-level actions
 * @returns Single formatted string with the compact hints
 */
export function compactHintsLine(keybindings: KeybindingsManager): string {
	const sendKey = editorKey("selectConfirm") || "enter";
	const stopKey = appKey(keybindings, "interrupt") || "ctrl+c";
	const clearKey = appKey(keybindings, "clear") || "ctrl+k";

	const hints = [
		bracketHint(sendKey, "send"),
		bracketHint(stopKey, "stop"),
		bracketHint("\u2191\u2193", "history"),
		bracketHint(clearKey, "clear"),
	];
