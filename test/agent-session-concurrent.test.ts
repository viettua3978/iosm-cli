...
describe("AgentSession concurrent prompt guard", () => {
	...
	it("should deliver meta as steering during regular streaming runs", async () => {
		createSession();
		...
		await session.meta("Meta message");
	...
	it("should deliver meta as steering while task orchestration is active", async () => {
		createSession();
		...
		// Simulate active task tool execution.
		(session as unknown as { _subagentStartMeta: Map<string, unknown> })._subagentStartMeta.set("task-call-1", {
			startedAt: Date.now(),
			profile: "full",
			description: "Running subtask",
		});

		await session.meta("Meta message during task");

		expect(steerSpy).toHaveBeenCalledTimes(1);
		expect(followUpSpy).toHaveBeenCalledTimes(0);

		(session as unknown as { _subagentStartMeta: Map<string, unknown> })._subagentStartMeta.clear();
		steerSpy.mockRestore();
		followUpSpy.mockRestore();
		await session.abort();
		await firstPrompt.catch(() => {});
