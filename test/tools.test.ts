...
describe("Coding Agent Tools", () => {
	...
	beforeEach(() => {
		...
		testDir = makeTempDir("coding-agent-test");
	...
	describe("bash tool", () => {
		...
		it("should sanitize ansi escape sequences from output", async () => {
			const result = await bashTool.execute("test-call-13", {
	...
	describe("grep tool", () => {
		...
		it("should respect .gitignore outside git repositories", async () => {
			writeFileSync(join(testDir, ".gitignore"), "ignored.txt\n");
			writeFileSync(join(testDir, "ignored.txt"), "match\n");
			writeFileSync(join(testDir, "kept.txt"), "match\n");

			const result = await grepTool.execute("test-call-12b", {
				pattern: "match",
				path: testDir,
			});

			const output = getTextOutput(result);
			expect(output).toContain("kept.txt:1: match");
			expect(output).not.toContain("ignored.txt:1: match");
		});
	});
	...
	describe("find tool", () => {
		...
		it("should not apply nested ignore overrides inside already ignored directories", async () => {
			mkdirSync(join(testDir, "ignored"), { recursive: true });
			writeFileSync(join(testDir, ".gitignore"), "ignored/\n");
			writeFileSync(join(testDir, "ignored", ".gitignore"), "!kept.txt\n");
			writeFileSync(join(testDir, "ignored", "kept.txt"), "kept");
			writeFileSync(join(testDir, "visible.txt"), "visible");
			...
			expect(output).not.toContain("ignored/kept.txt");
		});

		it("should exclude .git and node_modules from default search", async () => {
			mkdirSync(join(testDir, ".git"));
			mkdirSync(join(testDir, "node_modules", "pkg"), { recursive: true });
			mkdirSync(join(testDir, "src"));
			writeFileSync(join(testDir, ".git", "secret.txt"), "secret");
			writeFileSync(join(testDir, "node_modules", "pkg", "dep.txt"), "dep");
			writeFileSync(join(testDir, "src", "kept.txt"), "kept");

			const result = await findTool.execute("test-call-15", {
				pattern: "**/*.txt",
				path: testDir,
			});

			const output = getTextOutput(result);
			expect(output).toContain("src/kept.txt");
			expect(output).not.toContain(".git/secret.txt");
			expect(output).not.toContain("node_modules/pkg/dep.txt");
		});
	});
	...
	describe("ls tool", () => {
		...
		it("should avoid per-entry stat calls when detailed directory entries are available", async () => {
			let statCalls = 0;
			const customLsTool = createLsTool(testDir, {
				operations: {
					exists: () => true,
					stat: () => {
						statCalls += 1;
					...
					readdir: () => {
						throw new Error("fallback readdir should not be used");
					},
				},
			});

			const result = await customLsTool.execute("test-call-16", { path: testDir });
			const output = getTextOutput(result);
...
describe("edit tool fuzzy matching", () => {
	...
	beforeEach(() => {
		testDir = makeTempDir("coding-agent-fuzzy-test");
		mkdirSync(testDir, { recursive: true });
	});
...
describe("edit tool CRLF handling", () => {
	...
	beforeEach(() => {
		testDir = makeTempDir("coding-agent-crlf-test");
		mkdirSync(testDir, { recursive: true });
	});
