...
import { buildIosmContextBlock } from "../iosm/context-loader.js";
...
import { emptyHooksConfig, loadHooksConfig, type LoadedHooksConfig } from "./hooks.js";
...
export type ContextProfile = "standard" | "iosm";
...
function normalizeContextProfile(profile: string | undefined): ContextProfile {
	return profile?.toLowerCase() === "iosm" ? "iosm" : "standard";
}
...
function loadContextFilesFromDir(
	dir: string,
	options: { includeAgents?: boolean; includeIosm?: boolean } = {},
): Array<{ path: string; content: string }> {
	const includeAgents = options.includeAgents !== false;
	const includeIosm = options.includeIosm === true;
	const candidates = [
	...
	for (const filename of candidates) {
		...
		if (existsSync(filePath)) {
			try {
				...
				files.push({
					path: filePath,
					content: filename === "IOSM.md" ? buildIosmContextBlock(content) : content,
				});
			} catch (error) {
...
function listMarkdownFilesRecursive(dir: string): string[] {
	...
	for (const entry of entries) {
		...
		if (stat.isDirectory()) {
			files.push(...listMarkdownFilesRecursive(filePath));
			continue;
		}
...
function loadExpandedMemoryFile(
	filePath: string,
	maxDepth: number = 5,
	...
	const baseDir = resolve(resolvedPath, "..");
	...
	for (const line of lines) {
		...
		const importTargetRaw = importMatch[1]?.trim();
		if (!importTargetRaw) continue;
		const importTarget = importTargetRaw.replace(/^["']|["']$/g, "");
		const importPath = resolve(baseDir, importTarget);
		const imported = loadExpandedMemoryFile(importPath, maxDepth - 1, visited);
		if (imported && imported.trim().length > 0) {
			expanded.push(imported.trimEnd());
		}
	}
...
function loadMemoryContextFilesFromConfigDir(configDir: string): Array<{ path: string; content: string }> {
	const result: Array<{ path: string; content: string }> = [];
	const memoryRootFile = join(configDir, "memory.md");
	const expandedRoot = loadExpandedMemoryFile(memoryRootFile);
	if (expandedRoot && expandedRoot.trim().length > 0) {
		result.push({ path: memoryRootFile, content: expandedRoot });
	}

	const memoryDir = join(configDir, "memory");
	if (!existsSync(memoryDir)) return result;
	for (const filePath of listMarkdownFilesRecursive(memoryDir)) {
		const expanded = loadExpandedMemoryFile(filePath);
		if (!expanded || expanded.trim().length === 0) continue;
		result.push({ path: filePath, content: expanded });
	}

	return result;
}

function loadMemoryContextFilesFromRoot(rootDir: string): Array<{ path: string; content: string }> {
	return loadMemoryContextFilesFromConfigDir(join(rootDir, ".iosm"));
}

function loadMemoryContextFilesFromAgentDir(agentDir: string): Array<{ path: string; content: string }> {
	const files: Array<{ path: string; content: string }> = [];
	const seen = new Set<string>();
	const addUnique = (entry: { path: string; content: string }) => {
	...
	for (const entry of loadMemoryContextFilesFromConfigDir(agentDir)) addUnique(entry);
	for (const entry of loadMemoryContextFilesFromRoot(agentDir)) addUnique(entry);
	return files;
}
...
function loadProjectContextFiles(
	options: {
		...
		contextProfile?: ContextProfile | string;
	} = {},
): Array<{ path: string; content: string }> {
	...
	const appendUnique = (entry: { path: string; content: string }) => {
		if (seenPaths.has(entry.path)) return;
		contextFiles.push(entry);
		seenPaths.add(entry.path);
	};

	if (resolvedOrganizationDir) {
		for (const organizationContext of loadContextFilesFromDir(resolvedOrganizationDir, { includeIosm })) {
			appendUnique(organizationContext);
		}
		for (const organizationMemory of loadMemoryContextFilesFromRoot(resolvedOrganizationDir)) {
			appendUnique(organizationMemory);
		}
	}

	const userConfigDir = join(resolvedHomeDir, ".iosm");
	for (const userContext of loadContextFilesFromDir(userConfigDir, { includeIosm })) {
		appendUnique(userContext);
	}
	for (const userMemory of loadMemoryContextFilesFromRoot(resolvedHomeDir)) {
		appendUnique(userMemory);
	}

	for (const globalContext of loadContextFilesFromDir(resolvedAgentDir, { includeIosm })) {
		appendUnique(globalContext);
	}
	for (const globalMemory of loadMemoryContextFilesFromAgentDir(resolvedAgentDir)) {
		appendUnique(globalMemory);
	}
	...
	while (true) {
		const contextFilesInDir = loadContextFilesFromDir(currentDir, { includeIosm });
		for (let index = contextFilesInDir.length - 1; index >= 0; index--) {
		...
		const memoryFilesInDir = loadMemoryContextFilesFromRoot(currentDir);
		for (let index = memoryFilesInDir.length - 1; index >= 0; index--) {
			const memoryFile = memoryFilesInDir[index];
			if (seenPaths.has(memoryFile.path)) continue;
			ancestorContextFiles.unshift(memoryFile);
			seenPaths.add(memoryFile.path);
		}
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private organizationDir?: string;
	...
	private hooks: LoadedHooksConfig;
	...
	constructor(options: DefaultResourceLoaderOptions) {
		...
		this.organizationDir = options.organizationDir;
		this.contextProfile = normalizeContextProfile(options.contextProfile);
	...
	async reload(): Promise<void> {
		...
		const agentsFiles = {
			agentsFiles: loadProjectContextFiles({
				...
				organizationDir: this.organizationDir,
