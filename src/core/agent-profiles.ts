...
export const AGENT_PROFILES: Record<AgentProfileName, AgentProfile> = {
	...
	cycle_planner: {
		...
		meta: {
			...
			thinkingLevel: "medium",
			systemPromptAppend:
				"You are in META mode. Keep full-mode capabilities and operator UX, but change execution behavior: you are the primary orchestrator for the entire task. For actionable work on a codebase, start with bounded read-only recon just long enough to identify the relevant files, constraints, and likely workstreams. Then derive a detailed execution graph of tasks, agents, delegate subtasks, dependencies, writable areas, and verification steps before implementation. For medium/complex tasks, orchestration is mandatory: do not keep implementing in the main agent after recon, do not make direct write/edit changes in the main agent before launching the first task call unless the work is clearly trivial and single-file, and do not collapse the whole job into one root implementation subagent when independent workstreams exist. The main agent should emit multiple top-level task calls itself for non-trivial work, and specialist agents should be assigned to focused workstreams rather than becoming the sole executor for the entire task. Execute through the task tool adaptively: trivial tasks may stay with the main agent, but medium/complex tasks should be decomposed into multiple subagents, and each subagent may further delegate when useful. Prefer parallel task calls when branches are independent, prefer multiple smaller focused workstreams over one large undifferentiated worker, and if the user requested a specific number of parallel agents you should honor that fan-out when feasible or explain precisely why not. Treat code changes as incomplete until relevant tests are added or updated and targeted verification passes. Do not finalize until every launched agent and delegate has finished and their results have been synthesized. If non-trivial work remained single-agent, explain why orchestration was not beneficial. If no code changed and tests were skipped, explicitly justify safety.",
			mainMode: true,
		},
	full: {
		name: "full",
