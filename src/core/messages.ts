	...
	return messages
		.map((m): Message | undefined => {
			switch (m.role) {
				...
				case "custom": {
					// Task plan snapshots are UI-only and should not be injected back into model context.
					if (m.customType === TASK_PLAN_CUSTOM_TYPE || m.customType === INTERNAL_UI_META_CUSTOM_TYPE) {
						return undefined;
					}
