...
class SessionList implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		const scopeLabel = this.showCwd ? "all workspaces" : "current folder";
		const sortLabel = this.sortMode === "threaded" ? "threaded" : this.sortMode === "recent" ? "recent" : "relevance";
		const searchLabel = theme.fg("muted", "Search sessions");
		const summaryParts = [`Showing ${filteredCount}/${totalSessions}`, scopeLabel, sortLabel];
		if (this.nameFilter === "named") {
			summaryParts.push("named only");
		}

		// Render search input
		lines.push(truncateToWidth(searchLabel, width, "…"));
		lines.push(...this.searchInput.render(width));
		lines.push(truncateToWidth(theme.fg("muted", summaryParts.join(" • ")), width, "…"));
		...
		if (this.filteredSessions.length === 0) {
			...
			if (this.nameFilter === "named") {
				...
				if (this.showCwd) {
					emptyMessage = `  No named sessions found. Press ${toggleKey} to show all sessions.`;
				} else {
					emptyMessage = `  No named sessions in current folder. Press ${toggleKey} to show all, or Tab to search every workspace.`;
				}
			} else if (this.showCwd) {
				// "All" scope - no sessions anywhere that match filter
				emptyMessage = "  No sessions found. Adjust the search or press Esc to close.";
			} else {
				// "Current folder" scope - hint to try "all"
				emptyMessage = "  No sessions in current folder. Press Tab to search every workspace.";
			}
	...
	handleInput(keyData: string): void {
		...
		if (this.confirmingDeletePath !== null) {
			...
			// Allow both Escape and Ctrl+C to cancel (consistent with iosm UX)
...
export class SessionSelectorComponent extends Container implements Focusable {
	...
	private enterRenameMode(sessionPath: string, currentName: string | undefined): void {
		...
		panel.addChild(new Text(theme.fg("muted", `Path: ${shortenPath(sessionPath)}`), 1, 0));
		panel.addChild(
			new Text(
				theme.fg("muted", `Current name: ${currentName?.trim() ? currentName : "(unnamed session)"}`),
				1,
				0,
			),
		);
